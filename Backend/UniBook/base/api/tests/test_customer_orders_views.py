...
class CreateCustomerOrder(APITestCase):
    ...
    def test_create_customer_order_with_items_on_cart_with_valid_token(self):
    ...
    def test_create_customer_order_with_items_on_cart_with_invalid_token(self):
        CartItem.objects.create(book= self.book_1, 
                                quantity = 1,
                                cart = self.cart)
    ...
    def test_create_customer_order_with_items_on_cart_without_token(self):
        CartItem.objects.create(book= self.book_1, 
                                quantity = 1,
                                cart = self.cart)
    ...
    def test_create_customer_order_without_items_on_cart_with_valid_token(self):
        self.client.credentials(HTTP_AUTHORIZATION='Bearer ' + self.token)
        response = self.client.post(self.url)
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)

        response_data = json.loads(response.content)
        expected_data = {'detail': 'Cannot create an order without cart items'}
        self.assertEqual(response_data, expected_data)
...
class GetCustomerOrders(APITestCase):
    ...
    def test_get_customer_orders_where_customer_has_orders_with_valid_page_with_valid_token(self):
        order = Order.objects.create_order(customer= self.customer)
        OrderItem.objects.create_order_items(order= order,
                                            cart_items= self.customer.shopping_cart.cart_items.all())
...
class GetCustomerOrderInfo(APITestCase):
    def setUp(self):
        ...
        self.order = Order.objects.create_order(customer= self.customer)
